...
def pair_evaluate(env, agent_upswsing, agent_extended, n_games=1, device='cpu', render=False):
    """Plays an a game from start till done use two models, returns per-game rewards """
    agent_upswsing.train(False)
    agent_extended.train(False)
    game_rewards = []
    pair_game_rewards = []
    pos_ball = np.random.uniform(-0.5, 0.5, n_games)

    for i in range(n_games):
        state,_ = env.reset()
        total_reward = 0
        env.draw_ball(pos_ball[i], radius=0.01)
        while True:
            state = torch.tensor(state, dtype=torch.float32, device=device)
            (mus, sigmas), _ = model_upswing(s)
            dist = torch.distributions.MultivariateNormal(mus, torch.diag_embed(sigmas, 0))
            action = dist.sample().cpu().detach().numpy()
    ...
    for i in range(n_games):
        ...
        while True:
            ...
            if done or trunc:
                break
        pair_game_rewards.append(total_reward)
        
    agent_upswsing.train(True)
    agent_extended.train(True)
    return np.array(game_rewards), np.array(pair_game_rewards)
